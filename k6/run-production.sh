...
# ========================================
# 운영 환경 (Redis + Kafka) 테스트 실행 스크립트
# ========================================
#
# 사용법:
#   ./k6/run-production.sh [scenario]
#
# 시나리오:
#   all           - 전체 테스트 (기본값)
#   ranking       - 인기 상품 조회
...
#   ./k6/run-production.sh ranking      # 인기 상품 조회만
#   ./k6/run-production.sh coupon       # 쿠폰 발급만
#
...
check_app() {
    ...
    if curl -s -o /dev/null -w "%{http_code}" "${BASE_URL}/products" | grep -q "200\|404"; then
        ...
        echo "   docker-compose up -d && ./gradlew bootRun"
        return 1
    fi
}
