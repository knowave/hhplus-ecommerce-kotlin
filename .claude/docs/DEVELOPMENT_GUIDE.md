...
1. Kafka를 통해서 이벤트 체이닝으로 아래 순서 보장.
  * 주문 -> 결제
2. Coupon 발급 이벤트 추가
3. 재시도 3회 이상 실패 시 DLQ Consumer에 담고, DB에 담아 수동으로 처리.
...
13. 테스트 코드 환경 설정 시 테스트용 application.yml 을 사용해서 추가 및 수정 진행. 
...
* 인기 상품 조회 테스트 시나리오
  ...
  * product를 주문하고 결제처리.
  * product 인기 상품을 조회.
  * 이 과정을 50명의 user가 random으로 생성되어 있는 상품을 주문하고 결제하여 조회를 함.
* 쿠폰 발급 플로우
  * 100명의 user 생성
  * 쿠폰을 조회하고 랜덤으로 쿠폰을 발급받음
  * 100명의 user를 생성하고 한정적인 쿠폰을 동시에 쿠폰을 발급 받아서 동시성 제어가 비관적 락으로 처리 되는지 확인
* 주문과 결제
  * 100명의 user를 생성
  * 상품을 조회하고 해당 상품을 주문 생성 후 결제 처리
  * 재고가 없으면 예외처리 에러 발생
* SQL Script로 데이터를 생성하지 말고 API를 요청해서 처리를 할 것.