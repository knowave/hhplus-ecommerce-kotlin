...
### 3.3 쿠폰 발급: Redis 분산락 + DB 비관적락

#### 3.3.1 왜 2단계 방어가 필요한가?
...
override fun issueCoupon(couponId: UUID, request: IssueCouponCommand): IssueCouponResult {
    ...
    // 1단계: Redis 분산 락 획득 (외부 계층)
    return redisDistributedLock.executeWithLock(
        lockKey = lockKey,
        waitTimeMs = 3000,
        leaseTimeMs = 10000
    ) {
        // 2단계: DB 비관적 락으로 최종 검증 (내부 계층)
